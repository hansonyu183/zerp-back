...
本地联调可执行 `make seed-bob`，为客户、供应商、员工、产品、服务、仓库、车辆、资金账户、分类、部门和岗位写入 22 条演示数据，并覆盖 `EFFECTIVE`、`DRAFT`、`PENDING`、`REJECTED` 状态。演示数据先创建有效员工等参考对象，再创建带分类、部门、岗位、业务员、负责人和物流平台引用的对象；客户和供应商统一使用 `DEMO-EMP-001` 作为业务员，车辆归属自营物流平台。命令仅允许在 `development` 或 `test` 环境运行；重复执行会核验并跳过已有演示数据，不会覆盖同代码的其他内容。
