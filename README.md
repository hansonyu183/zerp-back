...
| 层级 | 含义 | 示例 |
| --- | --- | --- |
| `domain` | 业务领域，对应前端一级动态菜单 | `auth`、`vou` |
| `entity` | 业务实体，对应前端二级菜单及页面 | `user`、`saleorder` |
...
```text
POST /auth/user/signin   # 登录并写入会话 Cookie
POST /auth/user/session  # 恢复用户、菜单、权限及 CSRF Token
POST /auth/user/signout  # 注销并清理会话 Cookie
```
...
- `session` 接口返回当前用户、两级菜单、动作权限和 CSRF Token，具体数据结构由独立接口契约定义。
...
登录或恢复会话后，后端返回两级菜单及动作权限。标识与 API 路径保持一致：

- 一级菜单使用 `domain`；
- 二级菜单使用 `entity`；
- 动作权限使用对应实体允许的 `action` 集合；
- 前端页面路由使用 `/${domain}/${entity}`。

后端只返回稳定的业务标识、显示信息和权限数据，不返回可供前端直接导入的组件路径。前端菜单和按钮控制只用于改善交互，不构成安全边界。
...
具体 `domain`、`entity`、字段和动作编码以真实后端菜单及 API 契约为准。本文固定三级 API 与页面映射规则；当前已确定的销售订单示例为 `vou/saleorder/*`。
